...
/* harmony export */   toggleSpeechRecognition: () => (/* binding */ toggleSpeechRecognition)
/* harmony export */ });
...
function getPermissionsIframe() {
  return document.querySelector('#permissionsIFrame') || window.parent.document.querySelector('#permissionsIFrame');
}

// Function to request permission through the iframe
function requestPermissionThroughIframe() {
  var iframe = getPermissionsIframe();
  if (iframe) {
...
function toggleSpeechRecognition() {
  return _toggleSpeechRecognition.apply(this, arguments);
}
function _toggleSpeechRecognition() {
  _toggleSpeechRecognition = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
    var micButton;
...
function speak(text) {
  ...
  if (voices.length > 0) {
    ...
    var systemVoice = voices.find(function (voice) {
      return voice.name.includes('System') || voice.name.includes('Default') || voice.name.includes('Siri');
    });
    if (systemVoice) {
...
window.addEventListener('load', function () {
  ...
  var savedMode = localStorage.getItem('conversationMode');
  if (savedMode) {
  ...
  document.getElementById('conversationMode').addEventListener('change', function (e) {
...
/******/ })()
;
//# sourceMappingURL=speechHandler.bundle.js.map